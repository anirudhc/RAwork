...
#ifdef NFNETLINKQ
extern u_int16_t nfqueue_num;
extern struct nfq_handle *nfqh;
extern struct nfq_q_handle *qhndl;
#endif /* NFNETLINKQ */
...
#else
            OrderRuleLists("activation dynamic pass drop sdrop reject reinject alert log");
#endif /* IPFW */
#else
            if(InlineMode())
...
int ShowUsage(char *progname)
{
...
#ifdef NFNETLINKQ
    FPUTS_UNIX ("        -H <q-num> NFNETLINK QUEUE this snort listens on (0-65535) - multiple snorts\n");
#endif /* NFNETLINKQ */
    ...
    FPUTS_UNIX ("   --queue-maxlen                  Set the maximal length of the nfnetlink queue if the kernel supports it.\n");
...
#ifndef IPFW
#ifdef NFNETLINKQ
static char *valid_options = "?A:bB:c:CdDefF:g:G:h:H:i:Ik:K:l:L:m:n:NoOpP:qQr:R:sS:t:Tu:UvVwXyzZ:";
#else
...
#endif
#ifdef HAVE_NFQ_MAXLEN
   {"queue-maxlen", LONGOPT_ARG_REQUIRED, NULL, QUEUE_MAXLEN},
#endif
   {0, 0, 0, 0}
};
...
void CleanExit(int exit_val)
{
    ...
    if (InlineMode())
    {
        ...
        {
            LogMessage("Going to try to restore iptables rules %s\n",restorecmd);
             
            if(system(restorecmd) !=0)
            {
                LogMessage("iptables restore cmd failed restore manually\n");
            }
            else
            {
               LogMessage("iptables rules restored ok now oink oink exit\n");
            }
        }
