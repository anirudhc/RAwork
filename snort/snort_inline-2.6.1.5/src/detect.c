...
extern ListHead RejectBoth;
extern ListHead RejectSrc;
extern ListHead RejectDst;
#ifdef IPFW
extern ListHead Reinject;
#endif /* IPFW */
#endif /* GIDS */
...
void CreateDefaultRules()
{
    ...
    CreateRuleType("rejectboth", RULE_REJECTBOTH, 1, &RejectBoth);
    CreateRuleType("rejectsrc", RULE_REJECTSRC, 1, &RejectSrc);
    CreateRuleType("rejectdst", RULE_REJECTDST, 1, &RejectDst);
#ifdef IPFW
    CreateRuleType("reinject", RULE_REINJECT, 1, &Reinject);
#endif /* IPFW */
#endif /* GIDS */
...
int RejectBothAction(Packet * p, OptTreeNode * otn, Event *event)
{
    DEBUG_WRAP(DebugMessage(DEBUG_DETECT,
               "        <!!>Ignoring! \"%s\"\n",
               otn->sigInfo.message););
...
int RejectSrcAction(Packet * p, OptTreeNode * otn, Event *event)
{
    DEBUG_WRAP(DebugMessage(DEBUG_DETECT,
               "        <!!>Ignoring! \"%s\"\n",
               otn->sigInfo.message););
...
int RejectDstAction(Packet * p, OptTreeNode * otn, Event *event)
{
    DEBUG_WRAP(DebugMessage(DEBUG_DETECT,
               "        <!!>Ignoring! \"%s\"\n",
               otn->sigInfo.message););
...
#ifdef IPFW
int ReinjectAction(Packet * p, OptTreeNode * otn, Event *event)
{
    DEBUG_WRAP(DebugMessage(DEBUG_DETECT,
               "        <!!>Ignoring! \"%s\"\n",
               otn->sigInfo.message););

    // Let's log/alert, drop the packet, and mark it for reinject
    CallAlertFuncs(p, otn->sigInfo.message, otn->rtn->listhead, event);
