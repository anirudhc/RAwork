...
extern OutputFuncNode *RejectBothList;
extern OutputFuncNode *RejectSrcList;
extern OutputFuncNode *RejectDstList;
#ifdef IPFW
extern OutputFuncNode *ReinjectList;
#endif /* IPFW */
#endif /* GIDS */
...
void ParseRule(FILE *rule_file, char *prule, int inclevel)
{
    ...
    switch(rule_type)
    {
        ...
        case RULE_REJECTBOTH:
            DEBUG_WRAP(DebugMessage(DEBUG_CONFIGRULES,"RejectBoth\n"););
        ...
        case RULE_REJECTSRC:
            DEBUG_WRAP(DebugMessage(DEBUG_CONFIGRULES,"RejectSrc\n"););
        ...
        case RULE_REJECTDST:
            DEBUG_WRAP(DebugMessage(DEBUG_CONFIGRULES,"RejectDst\n"););
...
#ifdef IPFW
        case RULE_REINJECT:
            DEBUG_WRAP(DebugMessage(DEBUG_CONFIGRULES,"Reinject\n"););
              
            /* if we are not in inline mode, let's ignore
             * any reject rules in the configuration file */
            if (!InlineMode())
            {
                return;
    ...
    switch(rule_type)
    {
        ...
        case RULE_REJECT:
         ...
         case RULE_REJECTBOTH:
            if (InlineMode())
            {
                ProcessHeadNode(&proto_node, &RejectBoth, protocol);
            }
            break;
         ...
         case RULE_REJECTSRC:
            if (InlineMode())
            {
                ProcessHeadNode(&proto_node, &RejectSrc, protocol);
            }
            break;
         ...
         case RULE_REJECTDST:
            if (InlineMode())
            {
                ProcessHeadNode(&proto_node, &RejectDst, protocol);
            }
            break;
        ...
        case RULE_REINJECT:
            if (InlineMode())
            {
                ProcessHeadNode(&proto_node, &Reinject, protocol);
            }
            break;
#endif /* IPFW */
#endif /* GIDS */         
...
int RuleType(char *func)
{
    ...
    if (!strcasecmp(func, "rejectboth"))
    {
       if( pv.treat_drop_as_alert )
            return RULE_ALERT;
       else
            return RULE_REJECTBOTH;
    }
    if (!strcasecmp(func, "rejectsrc"))
    {
       if( pv.treat_drop_as_alert )
            return RULE_ALERT;
       else
            return RULE_REJECTSRC;
    }
    if (!strcasecmp(func, "rejectdst"))
    {
       if( pv.treat_drop_as_alert )
            return RULE_ALERT;
       else
            return RULE_REJECTDST;
    }
#ifdef IPFW
    if (!strcasecmp(func, "reinject"))
        return RULE_REINJECT;
#endif /* IPFW */
#endif /* GIDS */ 
...
void ParseConfig(char *rule)
{
    ...
    else if(!strcasecmp(config, "ipfw_reinject_rule"))
    {   
        if(args)
        {
            toks = mSplit(args, " ", 1, &num_toks, 0);
            ...
            pv.ipfw_reinject_rule = atoi(toks[0]) - 1;
